...
namespace Snake
{
    public class Leader_Board
    {
        public Leader_Board(int score)
        {
            ...
            StreamWriter file=new StreamWriter(@"C:\\LeaderBorad.txt",true); //Записываем файл на С диск
            file.WriteLine(name + " - " + score); // Записываем имя игрок и его счёт, через тире
            file.Close(); //Закрываем файл
            var fileToOpen = @"C:\\LeaderBorad.txt"; //Указываем Директория где лежит файл
